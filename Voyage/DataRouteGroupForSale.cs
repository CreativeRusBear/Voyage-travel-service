...
namespace Voyage
{
    public partial class DataRouteGroupForSale : Form
    {
        ...
        {
            adapter = new SqlDataAdapter("SELECT tGroupsRoutes.ID_Route, tGroupsRoutes.ID_Group, " +
                ...
                "WHERE tRoutes.ID_Route=" + this.idRoute, connection);
            dt = new DataTable();
            adapter.Fill(dt);
        ...
        //подготовка данных перед подсчетом свободных мест
        ...
        void LoadDataAboutCountOfPeople(int idGroup, int allCount, int countSale)
        {
            ...
            else MessageBox.Show("Извините, на данный момент все свободные места закончились", "Лимит свободных мест", MessageBoxButtons.OK, MessageBoxIcon.Information);
            connection.Close();
            label3.Text = freeCount.ToString();
        }
